...
class News extends MicroGrid {
	...
	public function GetNews($news_id = '0', $limit_clause = '')
	{
		...
		if(!empty($news_id)) $sql .= 'WHERE '.$this->primaryKey.' = '.(int)$news_id.' ';
		else $sql .= 'WHERE language_id = \''.Application::Get('lang').'\' ';
	...
	{
		$sql = 'SELECT COUNT(*) as cnt FROM '.TABLE_NEWS.' WHERE language_id = \''.Application::Get('lang').'\' ';
