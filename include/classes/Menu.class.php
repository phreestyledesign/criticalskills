...
class Menu {
	...
	{	
		$output = '<select name="menu_id" id="menu_id" style="width:145px">';
	...
	public static function DrawMenu($menu_position = 'left', $draw = true)
	{
		...
		if($menus_count > 0) $output .= '<div id="column-'.$menu_position.'-wrapper" '.(($menus_count > 0) ? '' : '').'>';//phreestyle edit. Removed inline css
		...
		if($menu_position == 'left'){
			if(!$objLogin->IsLoggedIn() || Application::Get('preview') == 'yes'){	
				if(Modules::IsModuleInstalled('users') && (ModulesSettings::Get('users', 'allow_login') == 'yes')){
					if(Application::Get('user') != 'login'){
						$output .= Users::DrawLoginFormBlock(false);		
					}
				}
			}
		...
		if($menus_count > 0) $output .= '</div><!-- end column wrapper -->';
		$output .= '<br />';
	...
	public static function DrawTopMenu($draw = true)
	{
		...
		$home_css = $user_css = '';
		$nl = "\n";
		if(Application::Get('user') == 'login' || Application::Get('user') == 'my_account'){
			$user_css = 'active';
		...
		if(Modules::IsModuleInstalled('users')){
			if(ModulesSettings::Get('users', 'allow_login') == 'yes'){
				$output .= '<li>'.prepare_permanent_link('index.php?user=my_account', _MY_ACCOUNT, '', $user_css).'</li>'.$nl;
			}
		}			
	...
	public static function DrawFooterMenu($draw = true)
	{
		...
		if(Modules::IsModuleInstalled('users')){			
	 			if(ModulesSettings::Get('users', 'allow_login') == 'yes'){
				$output .= ' : '.prepare_permanent_link('index.php?user=my_account', _MY_ACCOUNT);
			}
		}			
