...
if(!$objLogin->IsLoggedInAsUser()){
	...
	if($task == 'update'){
		...
		$sql = 'SELECT * FROM '.TABLE_USERS.' WHERE email = \''.mysql_real_escape_string($email).'\' AND id != '.(int)$objLogin->GetLoggedID();
	...
	$objUsers = new Users();
	$user_info = $objUsers->GetInfoByID($objLogin->GetLoggedID());
