...
if($objLogin->IsLoggedInAsAdmin()){
    ...
    if($objLogin->IsLoggedInAs('owner', 'mainadmin') && (file_exists('install.php') || file_exists('install/'))){
    	$actions_msg[] = '<span class="required">'._INSTALL_PHP_EXISTS.'</span>';
    }

	if(SITE_MODE == 'development') $actions_msg[] = '<span class="required">'._SITE_DEVELOPMENT_MODE_ALERT.'</span>';
    
    if($objLogin->IsLoggedInAs('owner', 'mainadmin')){
		$arr_folders = array('images/upload/', 'images/flags/', 'images/banners/', 'images/gallery/', 'tmp/backup/', 'tmp/cache/', 'feeds/');
	...
	if(ModulesSettings::Get('users', 'reg_confirmation') == 'by admin'){
		$users_count = Users::AwaitingAprovalCount();
		if($users_count > 0){
			$actions_msg[] = str_replace('_COUNT_', $users_count, _USERS_AWAITING_MODERATION_ALERT);			
		}
	}	
    ...
    $msg .= _HOME_PAGE_WELCOME_TEXT.'</div>';
