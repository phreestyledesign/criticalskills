...
if(!$objLogin->IsLoggedIn() && (ModulesSettings::Get('users', 'allow_registration') == 'yes')){
	...
	$reg_confirmation = ModulesSettings::Get('users', 'reg_confirmation');
	$image_verification_allow = ModulesSettings::Get('users', 'image_verification_allow');
	$admin_alert_new_registration = ModulesSettings::Get('users', 'admin_alert_new_registration');
	...
	if($act == 'create'){
		...
		if($msg == ''){
			$sql = 'SELECT * FROM '.TABLE_USERS.' WHERE user_name = \''.mysql_real_escape_string($user_name).'\'';
			$result = database_query($sql, DATA_AND_ROWS);
			if($result[1] > 0){
				$msg = draw_important_message(_USER_EXISTS_ALERT, false);
			}else{			
				// check if email already exists                    
				$sql = 'SELECT * FROM '.TABLE_USERS.' WHERE email = \''.mysql_real_escape_string($email).'\'';
				$result = database_query($sql, DATA_AND_ROWS);
				if($result[1] > 0){
