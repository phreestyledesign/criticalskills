...
/**
 *	Class Search (for MicroCMS ONLY)
 *  -------------- 
...
class Search {
	 ...
	 * 		@param $draw
	 */
	public static function DrawQuickSearch($image_button = true, $draw = true)
	{	
		...
		$output = '<form id="search-form" name="frmQuickSearch" action="index.php?page=search" method="post">
			<div class="header_search">
				...
				if($image_button) $output .= '<img class="search_button" src="'.APPHP_BASE.'templates/'.Application::Get('template').'/images/sbutton.gif" onclick="appQuickSearch()" alt="'._SEARCH.'" />';
				else $output .= '<input class="search_button" type="button" onclick="appQuickSearch()" value="'._SEARCH.'" />';
			$output .= '</div>
			</form>';
