...
/**
 *	Class UserGroups (for MicroCMS ONLY)
 *  -------------- 
 *	Written by  : ApPHP
 *  Updated	    : 07.03.2011
 *
 ...
 *	__destruct              GetAllGroupsByUsers
 *	
 *	
 **/


class UserGroups extends MicroGrid {
	...
	{		
		...
		$this->primaryKey 	= 'id';
		$this->tableName 	= TABLE_USER_GROUPS;
		$this->dataSet 		= array();
		$this->error 		= '';
		$this->formActionURL = 'index.php?admin=mod_users_groups';
	 ...
	 */
	public static function GetAllGroupsByUsers()
	{
		$sql = 'SELECT ug.id, ug.name, ug.description,
					(SELECT COUNT(*) FROM '.TABLE_USERS.' u WHERE u.group_id = ug.id AND u.is_active = 1 AND u.email_notifications = 1 AND u.email != "") as users_count
