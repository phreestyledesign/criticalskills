...
}else if((Application::Get('user') != '') && file_exists('user/handlers/handler_'.Application::Get('user').'.php')){
	if(Modules::IsModuleInstalled('users')){	
		include_once('user/handlers/handler_'.Application::Get('user').'.php');
	}
...
if($objSettings->GetParameter('caching_allowed') && !$objLogin->IsLoggedIn()){
	...
	$c_user        = Application::Get('user');
	$c_admin       = Application::Get('admin');
	
	if(($c_page == '' && $c_user == '' && $c_admin == '') || 
...
echo "\n".'<!-- This page was generated by ApPHP MicroCMS Pro v'.CURRENT_VERSION.' (http://www.apphp.com/php-microcms/) -->';
?>